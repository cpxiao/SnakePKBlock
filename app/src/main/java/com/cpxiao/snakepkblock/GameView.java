...
public class GameView extends BaseSurfaceViewFPS {
     ...
     * 速度
     */
    private float speed, speedCommon, speedMin;
    /**
     * 精灵列表
     */
    //    private List<AutoSprite> mSpriteList = new ArrayList<>();
    //    private CopyOnWriteArrayList<AutoSprite> mSpriteList = new CopyOnWriteArrayList<>();
    private ConcurrentLinkedQueue<AutoSprite> mSpriteList = new ConcurrentLinkedQueue<>();

    private long mFrame = 0;//总共绘制的帧数

    private AutoSprite mLastSprite = new AutoSprite();
    ...
    protected void initWidget() {
        ...
        mLastSprite.setSpeedAndAngle(speed, 90);
    ...
    private void drawStarted(Canvas canvas, Paint paint) {
        ...
        Iterator<AutoSprite> iterator = mSpriteList.iterator();
        while (iterator.hasNext()) {
            AutoSprite autoSprite = iterator.next();
            ...
            if (!autoSprite.isDestroyed()) {
                ...
                if (autoSprite instanceof Block) {
                    paint.setColor(Extra.ColorExtra.getBlockColor(getContext(), ((Block) autoSprite).getLife()));
                }
    ...
    private void drawSpriteList(Canvas canvas, Paint paint, boolean isOnDraw) {
        for (AutoSprite autoSprite : mSpriteList) {
            if (autoSprite instanceof Block) {
                paint.setColor(Extra.ColorExtra.getBlockColor(getContext(), ((Block) autoSprite).getLife()));
            }
            if (isOnDraw) {
    ...
    private void createSpite() {
        ...
        for (int i = 0; i < maxCount; i++) {
            ...
            if (random < blockProbability) {
                ...
                block.setSpeedAndAngle(speed, 90);
                block.centerTo((0.5f + i) * blockWH, -0.5f * blockWH);
                mSpriteList.add(block);
                ...
                circle.setSpeedAndAngle(speed, 90);
                circle.centerTo((0.5f + i) * blockWH, -0.5f * blockWH);
                mSpriteList.add(circle);
    ...
    private void checkBoardCollided() {
        List<LineBoard> lineBoardList = getLineBoardList();

        for (LineBoard lineBoard : lineBoardList) {
            resetMinAndMax(lineBoard);
        }
    }

    private void checkCircleCollided() {
        List<Circle> circleList = getCircleList();
     ...
     * @return list
     */
    public List<LineBoard> getLineBoardList() {
        List<LineBoard> list = new ArrayList<>();
        for (AutoSprite autoSprite : mSpriteList) {
            if (!autoSprite.isDestroyed() && autoSprite instanceof LineBoard) {
                LineBoard lineBoard = (LineBoard) autoSprite;
                list.add(lineBoard);
            }
        }
        return list;
    ...
    public List<Circle> getCircleList() {
        ...
        for (AutoSprite autoSprite : mSpriteList) {
            if (!autoSprite.isDestroyed() && autoSprite instanceof Circle) {
                Circle lineBoard = (Circle) autoSprite;
                list.add(lineBoard);
            }
        }
        return list;
